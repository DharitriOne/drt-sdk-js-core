...
describe("test address", () => {
    let aliceBech32 = "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8";
    let bobBech32 = "moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk";
    ...
    it("should create address (custom hrp)", async () => {
        ...
        address = Address.fromHex(bobHex, "xmoa");
        assert.deepEqual(address.getPublicKey(), Buffer.from(bobHex, "hex"));
        assert.equal(address.getHrp(), "xmoa");
        assert.equal(address.toBech32(), "xmoa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruqgn3hez");
    });
    ...
    it("should throw error when invalid input", () => {
        ...
        assert.throw(
            () => new Address("moa1l453hd0gt5gzdp7czpuall8ggt2dcv5zwmfdf3sd3lguxseux2"),
            errors.ErrAddressCannotCreate,
        );
        assert.throw(
            () => new Address("xmoa1l453hd0gt5gzdp7czpuall8ggt2dcv5zwmfdf3sd3lguxseux2fskgws3j"),
            errors.ErrAddressCannotCreate,
        );
    });
    ...
    it("should validate the address without throwing the error", () => {
        ...
        assert.isFalse(Address.isValid("xmoa1l453hd0gt5gzdp7czpuall8ggt2dcv5zwmfdf3sd3lguxseux2fskgws3j"));
        assert.isFalse(Address.isValid("moa1l453hd0gt5gzdp7czpuall8ggt2dcv5zwmfdf3sd3lguxseux2"));
    });

    it("should check whether isSmartContract", () => {
        assert.isFalse(
            Address.fromBech32("moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8").isSmartContract(),
        );
        assert.isTrue(
            Address.fromBech32("moa1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqplllsxxctf0").isSmartContract(),
        );
        assert.isTrue(
            Address.fromBech32("moa1qqqqqqqqqqqqqpgqxwakt2g7u9atsnr03gqcgmhcv38pt7mkd94qhg3njm").isSmartContract(),
        );
    });

    it("should contract address", () => {
        const addressComputer = new AddressComputer();
        const deployer = Address.fromBech32("moa1j0hxzs7dcyxw08c4k2nv9tfcaxmqy8rj59meq505w92064x0h40q4737p3");
        ...
        assert.equal(contractAddress.toBech32(), "moa1qqqqqqqqqqqqqpgqhdjjyq8dr7v5yq9tv6v5vt9tfvd00vg7h40qhxc27r");
        ...
        assert.equal(contractAddress.toBech32(), "moa1qqqqqqqqqqqqqpgqde8eqjywyu6zlxjxuxqfg5kgtmn3setxh40q5tpk55");
    });

    it("should get address shard", () => {
        const addressComputer = new AddressComputer();

        let address = Address.fromBech32("moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8");
        let shard = addressComputer.getShardOfAddress(address);
        assert.equal(shard, 1);

        address = Address.fromBech32("moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk");
        shard = addressComputer.getShardOfAddress(address);
        assert.equal(shard, 0);

        address = Address.fromBech32("moa1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaqhr5l9h");
        shard = addressComputer.getShardOfAddress(address);
