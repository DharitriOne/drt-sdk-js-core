...
describe("test types", () => {
    ...
    it("should get fully qualified name", () => {
        assert.equal(new Type("foo").getFullyQualifiedName(), "dharitri-sdk:types:foo");
        assert.equal(new U32Type().getFullyQualifiedName(), "dharitri-sdk:types:u32");
        assert.equal(
            parser.parse("MultiResultVec<u32>").getFullyQualifiedName(),
            "dharitri-sdk:types:MultiResultVec<dharitri-sdk:types:u32>",
        );
        assert.equal(parser.parse("utf-8 string").getFullyQualifiedName(), "dharitri-sdk:types:utf-8 string");
        assert.equal(
            parser.parse("Option<u32>").getFullyQualifiedName(),
            "dharitri-sdk:types:Option<dharitri-sdk:types:u32>",
        );
        assert.equal(new ManagedDecimalType(8).getFullyQualifiedName(), "dharitri-sdk:types:ManagedDecimal*8*");
        assert.equal(
            new ManagedDecimalType("usize").getFullyQualifiedName(),
            "dharitri-sdk:types:ManagedDecimal*usize*",
        );
        assert.equal(
            new ManagedDecimalSignedType(8).getFullyQualifiedName(),
            "dharitri-sdk:types:ManagedDecimalSigned*8*",
        );
        assert.equal(
            new ManagedDecimalSignedType("usize").getFullyQualifiedName(),
            "dharitri-sdk:types:ManagedDecimalSigned*usize*",
        );
    });
